...
void bkgsig(std::string inputFile) {
  ...
  const int nDim1=3;
  int nbins1[nDim]={10,10,10};
  double xmin1[nDim]={0,0,0};
  double xmax1[nDim]={1,1,1};
  const int nDim2=2;
  int nbins2[nDim]={10,10};
  double xmin2[nDim]={0,0};
  double xmax2[nDim]={1,1};
  THnSparseD* hs = new THnSparseD("hs", "hs", nDim, nbins, xmin, xmax);
  THnSparseD* hs1 = new THnSparseD("hs1", "hs1", nDim1, nbins1, xmin1, xmax1); 
  THnSparseD* hs2 = new THnSparseD("hs2", "hs2", nDim2, nbins2, xmin2, xmax2);
  ...
  for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
    ...
    double subjetCSV1[3]={-1,-1,-1};
    double subjetCSV2[3]={-1,-1};
    for(int i=0; i<2; i++)
      {
	...
	for(int isub=0; isub < FATnSubSDJet[ijet]; isub++)
	  {
	    ...
	    subjetCSV1[vectorID]=    FATsubjetSDCSV[ijet][isub]; //exactly 3 sub-jets
	    subjetCSV2[vectorID]=    FATsubjetSDCSV[ijet][isub]; //exactly 2 sub-jets
	  }
      }
    hs->Fill(subjetCSV);
    hs1->Fill(subjetCSV1); //exactly 3 sub-jets
    hs2->Fill(subjetCSV2); //exactly 2 sub-jets
  ...
  hs->Write("subjetcsv");
  hs1->Write("subjetcsv1");
  hs2->Write("subjetcsv2");
  outfile->Write();
